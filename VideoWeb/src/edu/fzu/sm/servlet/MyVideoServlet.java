...
public class MyVideoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(area.equals(CONST.AREA_ALL)) {
            videos = videoService.queryAllByName(user.getName(),title, nPage, 5);
        }
        else{
            videos = videoService.querayAllByArea(user.getName(),title,area,nPage,5);
        }
        ...
        req.getRequestDispatcher("/jsp/users/MyVideo.jsp").forward(req,resp);
    }
