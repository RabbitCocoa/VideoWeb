...
<%@ page import="edu.fzu.sm.util.PublicUtil" %><%--
...
<head>
    ...
    window.location.href = "<%=bashPath%>/MyVideoServlet?page="+page+"&Area="+$area+"&Title="+$title;
    }
    ...
    function del(vid) {
        ...
        if (confirm("确定删除该视频吗?一旦删除无法恢复")) {
            $.post(
                "<%=bashPath%>/DelVideoServlet",
                "Vid="+vid,
                function (result) {
                    if (result == "success") {
                        alert("删除成功");
